			...
			<li>
				...
				<div class="li-text">
					...
					<div class="li-text2">
						...
						<p class="dd">직 종 :
						<c:choose>
						<c:when test="${myself.jobId=='job1001'}">경영·사무</c:when>
						<c:when test="${myself.jobId=='job1002'}">유통·무역</c:when>
						<c:when test="${myself.jobId=='job1003'}">영업·고객상담</c:when>
						<c:when test="${myself.jobId=='job1004'}">생산·제조</c:when>
						<c:when test="${myself.jobId=='job1005'}">IT·인터넷</c:when>
						<c:when test="${myself.jobId=='job1006'}">디자인</c:when>
						<c:when test="${myself.jobId=='job1007'}">전문직</c:when>
						<c:when test="${myself.jobId=='job1008'}">교육</c:when>
						<c:when test="${myself.jobId=='job1009'}">미디어</c:when>
						<c:when test="${myself.jobId=='job1010'}">특수계층·공공</c:when>
						<c:when test="${myself.jobId=='job1011'}">건설</c:when>
						<c:when test="${myself.jobId=='job1012'}">서비스</c:when>
						<c:when test="${myself.jobId=='job1013'}">의료</c:when>
						<c:when test="${myself.jobId=='job1014'}">기타</c:when>
						</c:choose>
						</p>
					</div>
					<div class="li-text3">
